...
| B16 | **Network filters block the domain**: a library computer gets ERR_CONNECTION_RESET | reported 2026-09-24 | Not ours to fix and nothing in our config causes it: TLS is clean and Cisco Umbrella, Quad9 and Cloudflare all resolve it normally, so it is the library's own appliance, most likely a newly-registered-domain rule (domain registered 17.9.2026, window closes ~17.10.2026). Fixed by submitting the domain for categorisation with the filter vendors and by asking the network owner to allowlist it. Both drafted in `docs/network-filter-block.md`; the vendor forms need a person because of the captchas. Matters because libraries and student networks are the channel B9 and the adoption plan aim at, and nobody reports a site that simply fails |
...
| E16: the typefaces read as another product's | **Closed by the v2 redesign (D-28), 2026-09-24.** Fraunces and Inter replace Newsreader, Public Sans and IBM Plex Mono, self-hosted, and the ground moved out of the cream family into the product's own green |
